...
#define MODULES_COUNT	6	// 2xInput, 2xMULT, 2xADD, 2xBITSHIFT
...
void test(double fCross, double fMut)
{
	...
	for (int i = 0; i < 2; i++)
		pModules[INPUTS_COUNT + i] = new CBitShifter();

	for (int i = 0; i < 2; i++)
		pModules[INPUTS_COUNT + 2 + i] = new CAdder();
	...
	BYTE instModAll[] = {0x00, 0x00, 0x00, 0x3F};
...
#define INSTR_COUNT (16)
	...
	CInstruction* arrInstrSet[INSTR_COUNT] = { 
									...
									new CInstruction(4, instOneAnd, false, std::string("ONE"), 1, iosOneAnd, 8),
									new CInstruction(4, instOneAnd, false, std::string("ONE"), 1, iosOneAnd, 8),*/
									new CInstruction(4, instModAll, false, std::string("ALL"), 1, iosModAll, 20),
									new CInstruction(4, instModAll, false, std::string("ALL"), 1, iosModAll, 20),
									new CInstruction(4, instModAll, false, std::string("ALL"), 1, iosModAll, 20),
									new CInstruction(4, instModAll, false, std::string("ALL"), 1, iosModAll, 20),
									new CInstruction(4, instModAll, false, std::string("ALL"), 1, iosModAll, 20),
	...
	for (UINT i = 0; i < 50; i++) {
		...
		CSigmoidPlanFramework * pFramework = new CSigmoidPlanFramework(pArch, pEnv, evoParams, indivParams);

		/*CSigmoidPlanIndiv tmpIndiv(pArch, pEnv, pFramework, &indivParams);
		std::ifstream inFile("d:\\EvoHSC\\sigmoid_plan\\indiv_out_1254E3.xml");
		std::stringstream buffer;
		buffer << inFile.rdbuf();
		std::string sXml = buffer.str();
		inFile.close();
		tmpIndiv.Parse(sXml);
		evoParams.strSimulParams.bPrintDebug = true;
...
double CSigmoidPlanIndiv::EvaluateOutputs(CEnvironment * pEnv)
{
	std::vector<REGISTER_TYPE> vecOuts;
	CDynamicTimeline * pTimeline = pEnv->GetOutput(0);
	if (pTimeline) {
		int nActOut = 0;
		for (strDynamicTimeVal * pVal = pTimeline->GetFirst(); pVal; pVal = pVal->pSucc, nActOut++) {
			vecOuts.push_back(pVal->val);
		}
	}

	double dMaxFitness = 0;
	for (std::vector<REGISTER_TYPE>::iterator itOut = vecOuts.begin(); itOut != vecOuts.end(); itOut++)
	{
		double dCurrFitness = 0;

		REGISTER_TYPE out = pOutputs[nActTestSet][0];

		if (*itOut == out)
			dCurrFitness = (100.0 / SAMPLES_COUNT);
		else
		{
			double dDiff = abs(*itOut - out) / 2.0;
			dCurrFitness += (100.0 / SAMPLES_COUNT) / pow(1 + dDiff, 2);
		}

		if (dCurrFitness > dMaxFitness)
			dMaxFitness = dCurrFitness;
	}

	//std::cout << "OUT: " << *vecOuts.begin() << std::endl;
	return dMaxFitness;
}

int CSigmoidPlanIndiv::PregenerateProgram()
{
	return 0;
...
bool CSigmoidPlanFramework::EvaluateStopCondition(UINT nGeneration)
{
	...
	for (UINT nBest = 0; nBest < m_strEvoParams.nEliteSize; nBest++) {
		...
		if (m_pBestIndivs[nBest]->GetFitness(eResultsFitness) >= 80) {
			if (true) {
				if (m_bPrintInfo) {
...
void CSigmoidPlanFramework::ResetAfterEval()
{
}

bool CSigmoidPlanFramework::IsNextInputSet()
{
